...
---@alias framework.gui.element_handler fun(e: framework.gui.event_data)
...
---@alias framework.gui.event_handler framework.gui.element_handler|table<defines.events, framework.gui.element_handler>
...
---@field handler framework.gui.event_handler? Handler(s) to assign to this element. If assigned to a function, that function will be called for any GUI event on this element.
...
---@field event_handlers table<defines.events, table<string, framework.gui.element_handler>>
...
function FrameworkGui:register_handlers(gui_element, extras)
    ...
    for event, handler in pairs(handlers --[[@as table<defines.events, framework.gui.element_handler>]]) do
...
---@param ev framework.gui.event_data
...
function FrameworkGui:dispatch(ev)
    ...
    local handler = self.event_handlers[event_id]
    assert(handler)
    if not handler[elem.name] then return false end

    handler[elem.name](ev)
    return true
end

------------------------------------------------------------------------
-- update, called by the ticker to update the gui state if it is open

---@return boolean refresh_ok Destroy gui entity if false
function FrameworkGui:update()
    if not self.update_callback then return true end
    return self.update_callback(self, self.context)
end
